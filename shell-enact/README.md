...
- `src/core/` is generated by `../tools/sync-core.js` from `../core` and is git-ignored.
- `pack`, `pack-p`, `serve` and `watch` pass `--no-linting` because the enact webpack ESLint plugin uses its own flat config that rejects the generated `src/core` files (the shared core is linted by the root `npm test`).
- The Sandstone template was installed with `npx enact template install @enact/template-sandstone` then `npx enact create -t sandstone shell-enact` (@enact/cli 7.3.4 only ships the limestone template by default).
